...
-- 1. Create a function to sync admin status to user metadata
...
-- 2. Create a trigger that fires when user_profiles.is_admin changes
...
-- 3. Also create a function to initialize admin status on user creation (first user becomes admin)
...
-- 4. Create trigger for initializing first user as admin
...
-- 5. Sync existing admin users to have the correct metadata
DO $$
DECLARE
  admin_user RECORD;
BEGIN
  ...
  SET
    ...
    updated_at = NOW()
  WHERE
    id IN (SELECT user_id FROM user_profiles WHERE is_admin = false OR is_admin IS NULL)
    AND raw_user_meta_data ? 'is_admin';
END $$;

-- 6. Add comments explaining the system
