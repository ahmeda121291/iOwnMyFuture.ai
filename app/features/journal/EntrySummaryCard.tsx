...
const EntrySummaryCard = React.memo(function EntrySummaryCard({ entry, onView, onEdit, onDelete, showViewLink = false }: EntrySummaryCardProps) {
  ...
  const { wordCount, readingTime, moodColor, moodLabel, preview } = useMemo(() => {
    ...
    let label = 'Neutral mood detected';
    
    if (positiveCount > negativeCount) {
      color = 'bg-green-400';
      label = 'Positive mood detected';
    } else if (negativeCount > positiveCount) {
      color = 'bg-yellow-400';
      label = 'Reflective mood detected';
    }
    
    const contentPreview = entry.content.length <= 180 
      ? entry.content 
      : entry.content.substring(0, 180).trim() + '...';
      ...
      <div onClick={() => onView(entry)} className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="text-sm font-semibold text-accent">{wordCount}</div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 ${moodColor} rounded-full`}></div>
              <span className="text-xs text-text-secondary">{moodLabel}</span>
            </div>
          </div>
