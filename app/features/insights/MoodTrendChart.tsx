...
const MoodIcon = React.memo(({ mood }: { mood: string }) => {
  switch (mood) {
    case 'positive':
...
const MoodLegendItem = React.memo(({ 
  ...
  const colorClass = mood === 'positive' ? 'text-green-600' : 
                    mood === 'negative' ? 'text-red-600' : 'text-yellow-600';
  
  return (
    <div className="flex items-center">
      <MoodIcon mood={mood} />
      <span className="ml-2 text-text-secondary capitalize">{mood}</span>
...
const InsightMessage = React.memo(({ trendDirection }: { trendDirection: string }) => (
...
function MoodTrendChart({ data, chartType = 'area' }: MoodTrendChartProps) {
  // Memoize expensive calculations
  const { positivePercentage, neutralPercentage, negativePercentage, trendDirection } = useMemo(() => {
    const total = data.reduce((sum, entry) => 
      sum + entry.positive + entry.neutral + entry.negative, 0
    );
    
    const positive = data.reduce((sum, entry) => sum + entry.positive, 0) / total * 100;
    const neutral = data.reduce((sum, entry) => sum + entry.neutral, 0) / total * 100;
    const negative = data.reduce((sum, entry) => sum + entry.negative, 0) / total * 100;

    const trend = positive > 50 ? 'improving' : 
                  negative > 50 ? 'declining' : 'stable';
    ...
    <div className="card">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-center space-x-6 text-sm">
          <MoodLegendItem mood="positive" percentage={positivePercentage} />
          <MoodLegendItem mood="neutral" percentage={neutralPercentage} />
          <MoodLegendItem mood="negative" percentage={negativePercentage} />
        </div>

        {/* Insight */}
        <InsightMessage trendDirection={trendDirection} />
      </div>
    </div>
  );
}
